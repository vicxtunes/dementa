// Chemistry curriculum — the 11 industrial processes, authored by hand.
// The structured shape below is chemistry-specific; chemistry/index.ts maps it
// onto the generic Topic type.

type SideEffect = { issue: string; effect: string; mitigation: string };
type Flashcard = { front: string; back: string };
type ChemQuizQuestion = {
  question: string;
  options: string[];
  correctIndex: number;
  explanation: string;
